...
void checkRadioChannels()
{
    ...
    for (channel = 0; channel < 256; channel++)
    {
        ...
        while (MARCSTATE != 1)
            ; //radio should already be idle, but check anyway
        CHANNR = channel;
...
void reportResults(int iterationFirst, int iterationLast)
{
    ...
    {
        printf(
            "%02X-%02X-%02X-%02X #%d, 0x%04X%04X ms [ ",
            (uint16)serialNumber[3],
            (uint16)serialNumber[2],
    ...
    {
        printf(
            "%02X-%02X-%02X-%02X #%d%04d, 0x%04X%04X ms [ ",
            (uint16)serialNumber[3],
            (uint16)serialNumber[2],
            (uint16)serialNumber[1],
            (uint16)serialNumber[0],
